...
class STN(nn.Module):
    ...
    def forward(self, x):
        ...
        if not self.quaternion:
            iden = Variable(torch.from_numpy(np.identity(self.dim, 'float32')).clone()).view(1,
                                                                                             self.dim * self.dim).repeat(
                batchsize, 1)
...
class PointNetFeat(nn.Module):
    def __init__(self, num_scales=1, num_points=500, use_point_stn=True, use_feat_stn=True, sym_op='max',
        ...
        if self.use_point_stn:
            self.stn1 = STN(num_scales=self.num_scales, num_points=num_points * self.point_tuple, dim=3,
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, conv=False):
